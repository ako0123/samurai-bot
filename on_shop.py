...
token = '1289131326:AAGXIawwewqt70_8M535p22mjnQXQE03mYc'
bot = telebot.TeleBot(token)
markup = types.InlineKeyboardMarkup(row_width=2)
food1 = types.InlineKeyboardButton("Филадельфия", callback_data='food1')
food2 = types.InlineKeyboardButton("Цезарь ролл", callback_data='food2')
food3 = types.InlineKeyboardButton("Суши унаги", callback_data='food3')
...
def order(message):
    bot.send_message(message.chat.id, text='Рахмет за ваш заказ, отправьте пожалуйста ваш адрес и мы отправим к вам курьера')
...
def data (call):
    if call.message:
        if call.data == 'food1':
            ...
            food1=open('food1.jpg', 'rb')
            bot.send_photo(call.message.chat.id,food1)
            bot.send_message(call.message.chat.id, text='Лосось, сыр сливочный, огурец, нори, рис.', reply_markup=markup1)
            ...
            bot.send_message(call.message.chat.id, text='<b> Меню </b>', parse_mode='html', reply_markup=markup)
        elif call.data == 'add1':
            mark=types.ReplyKeyboardMarkup(resize_keyboard=True)
            food1 = types.KeyboardButton("Филадельфия")
...
def message(message):
    if message.text=="Филадельфия":
        update_order(message.chat.id, "Филадельфия",'1399')
        ...
        bot.send_message(message.chat.id,text='Ваш заказ{}, ваш счет {}'.format(get_order(message.chat.id),get_bill(message.chat.id)))
